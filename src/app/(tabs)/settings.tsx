...
function SettingRow({ icon, title, detail, action }: { icon: keyof typeof Ionicons.glyphMap; title: string; detail?: string; action: React.ReactNode }) { const { colors } = useTheme(); const styles = useMemo(() => createStyles(colors), [colors]); return <View style={styles.row}><View style={styles.icon}><Ionicons name={icon} size={21} color={colors.forest} /></View><View style={styles.rowCopy}><Text style={styles.rowTitle}>{title}</Text>{detail ? <Text style={styles.rowDetail}>{detail}</Text> : null}</View>{action}</View>; }
...
const createStyles = (colors: AppPalette) => StyleSheet.create({ root: { flex: 1, backgroundColor: colors.cream }, content: { paddingTop: 64, paddingHorizontal: 20, paddingBottom: 40 }, kicker: { color: colors.moss, fontSize: 10, fontWeight: '700', letterSpacing: .7 }, title: { marginTop: 7, color: colors.forestDark, fontFamily: 'serif', fontSize: 32, fontWeight: '700' }, lead: { marginTop: 9, marginBottom: 24, color: colors.muted, fontSize: 13, lineHeight: 19 }, group: { marginBottom: 15, paddingHorizontal: 14, borderWidth: 1, borderColor: colors.line, borderRadius: 22, backgroundColor: colors.paper }, row: { minHeight: 74, flexDirection: 'row', alignItems: 'center', gap: 11, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line }, icon: { width: 38, height: 38, alignItems: 'center', justifyContent: 'center', borderRadius: 13, backgroundColor: colors.sageSoft }, rowCopy: { flex: 1 }, rowTitle: { color: colors.ink, fontSize: 14, fontWeight: '700' }, rowDetail: { marginTop: 3, color: colors.muted, fontSize: 10 }, segment: { flexDirection: 'row', padding: 3, borderRadius: 11, backgroundColor: colors.sageSoft }, segmentButton: { minWidth: 36, minHeight: 32, alignItems: 'center', justifyContent: 'center', borderRadius: 8 }, themeButton: { width: 31, minHeight: 32, alignItems: 'center', justifyContent: 'center', borderRadius: 8 }, segmentActive: { backgroundColor: colors.primary }, segmentText: { color: colors.muted, fontSize: 10, fontWeight: '700' }, segmentTextActive: { color: colors.onPrimary }, delete: { minHeight: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, deleteText: { color: colors.danger, fontSize: 13, fontWeight: '700' }, disclaimer: { marginTop: 12, color: colors.muted, fontSize: 10, lineHeight: 15, textAlign: 'center' } });
